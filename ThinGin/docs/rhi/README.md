# ArcRHI architecture analysis

These documents summarize the current state of ArcRHI's Render Hardware Interface (RHI) effort, with a focus on building a C#-native, driver-centric RHI architecture.

## Scope

- **Included:** Code under `ArcRHI/Core/RenderHardware` and adjacent rendering-facing code that interacts with it.
- **Also covered:** The existing OpenGL backend (`ArcRHI.OpenGL`) and how it relates to (or diverges from) the newer RHI work.
...
- There is a **new RHI core surface area** in `ArcRHI/Core/RenderHardware` that already defines many modern RHI concepts (resource types, pipeline init structs, command list types, fences, etc.).
- There is also a **legacy (older) engine/provider model** in `ArcRHI.OpenGL/Common` that binds OpenGL directly and powers the existing examples.
