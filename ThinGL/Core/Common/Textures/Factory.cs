...
namespace ThinGin.Core.Common.Textures
{
    ...
    public static class Factory
    {
        ...
        /// <returns></returns>
        public static bool TryDecode(ReadOnlyMemory<byte> EncodedData, out byte[] DecodedData, out TextureMetadata Metadata)
        {
            foreach (var decoder in Decoders)
            {
                if (decoder.TryDecode(EncodedData, out byte[] outData, out TextureMetadata outMetadata))
                {
                    DecodedData = outData;
        ...
        public static TextureProxy Create(IRenderEngine Engine, string Name) => Create(Engine, Name, Engine.Get_Default_Texture_Internal_Pixel_Layout());
        ...
        /// <exception cref="System.IO.IOException"></exception>
        /// <exception cref="Exception"></exception>
        public static TextureProxy Create(IRenderEngine Engine, string Name, PixelDescriptor GpuLayout)
        {
            if (!Engine.TextureCache.IsCached(Name))
            {
                ...
                if (!TryDecode(RawData, out byte[] DecodedData, out TextureMetadata Metadata))
                {
                ...
                if (!Engine.TextureCache.TryRegister(Name, tex))
                {
                    throw new Exception($"Unable to register texture in cache! (Identifier: {Name})");
                }
            }

            if (!Engine.TextureCache.TryReference(Name, out Texture outRoot))
            {
